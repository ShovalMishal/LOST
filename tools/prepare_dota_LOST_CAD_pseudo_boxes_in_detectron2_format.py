...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description="Prepares the LOST pseudo-boxes from a DOTAV2"
                    "dataset in the data format expected from detectron2.")
    parser.add_argument("--dota_dir", type=str, default='/home/shoval/Documents/Repositories/data/multiscale_normalized_dataset_rotated/train',
                        help="Path to where the DOTA dataset is.")
    parser.add_argument("--pboxes", type=str, default='../outputs/DOTA_MS_NORMALIZED_train/LOST-vit_small16_k/preds.pkl',
                        help="Path to where the LOST CA pseudo boxes for the DOTAV2 data are.")
    args = parser.parse_args()

    print('Prepare LOST pseudo-boxes (DOTAV2) in the data format expected from detectron2.')
    ...
    for image_name in tqdm(LOST_pseudo_boxes):
        if 'jpg' in image_name:
            ...
            image_name = annot['images'][ann_id]['file_name']

        image_id = image_name.split('/')[-1].split('.')[0]
        # image_id_int = int(image_id)
        full_img_path = pathlib.Path(args.dota_dir) / "images" / image_name
    ...
    dataset_name = f"dota_multiscale_normalized_train_LOST_CAD"
    json_data = {
        "dataset": data,
        "meta_data": {
            "dirname": args.dota_dir,
